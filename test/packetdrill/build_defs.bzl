...
def _packetdrill_test_impl(ctx):
    ...
    runner_content = "\n".join([
        ...
        "%s %s --init_script %s -- %s\n" % (
            ...
            " ".join(ctx.attr.flags),
            ctx.files.init_script[0].short_path,
            " ".join(script_paths),
        ),
    ])
    ...
    if hasattr(ctx.attr.test_runner, "data_runfiles"):
        transitive_files = depset(ctx.attr.test_runner.data_runfiles.files)
    runfiles = ctx.runfiles(
        files = [test_runner] + ctx.files.init_script + ctx.files.scripts,
