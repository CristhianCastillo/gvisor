...
type Source interface {
	...
	//
	// Preconditions: r != nil. filter != 0. The ReceiverCallback for r must
	// not take locks that are ordered prior to the Source; for example, it
	// cannot call any Source methods.
...
// UnsubscribeAndClear is a convenience function that unsubscribes r from the
// given events from src and also clears them from r.
func UnsubscribeAndClear(src Source, r *Receiver, filter Set, id SubscriptionID) {
