...
func routeICMPv6Packet(t *testing.T, args routeArgs, fn func(*testing.T, header.ICMPv6)) {
	...
	if args.remoteLinkAddr != "" && args.remoteLinkAddr != pi.RemoteLinkAddress {
		t.Errorf("got remote link address = %s, want = %s", pi.RemoteLinkAddress, args.remoteLinkAddr)
	}
	...
	for {
		...
		if resCh != nil {
			...
			for _, args := range []routeArgs{
				{src: c.linkEP0, dst: c.linkEP1, typ: header.ICMPv6NeighborSolicit, remoteLinkAddr: header.EthernetAddressFromMulticastIPAddress(header.SolicitedNodeAddr(lladdr1))},
